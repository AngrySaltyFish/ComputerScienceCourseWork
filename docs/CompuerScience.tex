...
\begin{figure}
    \begin{tikzpicture}[node distance=3cm]
        ...
        \node (ifHtml) [decision, below of=start] {is the response 
                                                    empty or is html};

        \node (ifSigparse) [decision, below of=ifHtml] {Is signature
                                                        parsing needed};

        \node (ifJs) [decision, right of=ifHtml] {Is the response js};
        
        \draw [arrow] (ifHtml) -- node {Yes} (ifSigparse);
        \draw [arrow] (ifHtml) -- node {No} (ifJs);
    \end{tikzpicture}
\end{figure}
