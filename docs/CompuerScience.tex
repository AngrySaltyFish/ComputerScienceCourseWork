...
\tikzstyle{TextBox} = [rectangle, draw, fill=blue!20, 
    right, text width=6em, rounded corners]
...
The two item on the graph above which has the most visual significance would be DownloadTab
and LibraryTab as they are the direct descendants of the Tab Widget. Due to this fact I 
think that it is important to layout how I intend this two item to look. The first that we
will look at will be the Download Tab which is shown as Figure ~\ref{fig:DownloadTab Diagram}.
There are quite a few objects on this diagram which are not in the layer model (Fig ~\ref{fig:Layer Model})
these objects are to interact with the two main widgets on the DownloadTab which are preview
and searchlist. As the name suggestests the preview is to allow the user to view the video before
downloading while 
...
\begin{figure}
    ...
    \begin{tikzpicture}
        ...
        \node [TextBox, above=0cm of inherit, text width = 30em] (textbox) {
            The class will be used to play media selected by the user and provide controls. This
            will be on both DownloadTab and LibraryTab. The tree on the left describes what happens
            when you call the constructor. Since this is a class that will be visually displayed
            to the user this class inherits the QWidget.
        };
    \end{tikzpicture}
...
\begin{figure}
    ...
    \begin{tikzpicture}
        ...
        \node [TextBox, above=0cm of inherit, text width = 30em] (textbox) {
            This class will provide a canvas to add all of controls to download and preview videos.
            It will also act as a middle man for many of the classes so that if need be they can acess
            each other. The tree shows it's constructor which is initiated from the MainWindow class.
            Another note worthy feature is the fact it is inherited from TabWidget. This is so that it
            is automatically added as a tab.
        };
    \end{tikzpicture}
...
\begin{figure}
    ...
    \begin{tikzpicture}
        ...
        \node [TextBox, above=0cm of inherit, text width = 30em] (textbox) {
           This class handles all of the network request with the exception of searching for
           videos meaining that this class is completely backend. This means the user should
           never know about this class. I putting all of the networking into this class as 
           there are lots of problems that can occur and if this is setup correctly I will
           not need to repeat to handle errors. Due to the nature of networking this class
           has numerous signal and slots to make this a asychronous as possible.
        };
    \end{tikzpicture}
