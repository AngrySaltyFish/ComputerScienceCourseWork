...
\begin{figure}
    \begin{tikzpicture}[node distance=6cm]
        ...
        \draw [arrow] (ifQualityLinksIsNotEmpty) -| node {} (error);
...
\begin{figure}
    \begin{tikzpicture}[node distance=6cm]
        ...
        \node (whileLoop) [decision, fill=cyan!60, below of=start] {While there is an item in
            fmtQualities};
        \node (createVideoLink) [process, yshift=-3cm, hyperlink node=parseVideo4,
        ...
        \node (FinishMe) [process, below of=createVideoLink] {TODO://Finish Me!};
    \end{tikzpicture}
\end{figure}
...
\begin{figure}[h]
    \begin{tikzpicture}[node distance=6cm]
        ...
        \draw [arrow] (jsIsFalse) -- node {} (addDownload);
        \draw [arrow] (jsIsTrue) -- node {} (addDownload);
        \draw [arrow] (addDownload) -- +(4,0) |- node {} (titleRegex);
        \draw [arrow] (titleRegex) -- node {} (continue);
...
\begin{figure}[h]
    \begin{tikzpicture}[node distance=6cm]
        ...
        \node (start) [startstop] {\hypertarget{parseVideo3}{Continuation of parseVideo}};
        \node (ifRegexFoundTitle) [decision, below of=start] {Has the regex found the title};
        \node (ExtractTitle) [decision, below of=ifRegexFoundTitle] {extract the title and set the title property to it};
        \node (regexForQualityLinks1) [process, minimum width=10cm, below of=ExtractTitle] {Create the following regex: 
        regex \verb!"\"adaptive_fmts\": ?\"(.*)\"! and
        \verb!"\"url_encoded_fmt_stream_map\": ?\"(.*)\"!};
        \node (IfregexQualityLinks1) [decision, right of=regexForQualityLinks1] {If found a links add it to a list};
...
\begin{figure}[h]
    \begin{center}
        \begin{tabular} { | c | c | c |}
            ...
            \caption{Data table fig} \label{fig:dataTable}
        \end{tabular}
    \end{center}
\end{figure}
